/**
 * Script para n8n - Transformar JSON de Facebook a Google Sheets
 * 
 * USAGE EN N8N:
 ...
 * - Extrae SOLO Facebook del response
 */

// Obtener el JSON del request
const requestData = $input.first().json;

console.log('📊 Iniciando transformación de datos Facebook...');
console.log(`   Período: ${requestData.data?.period}`);

// Validar que el request fue exitoso y tiene Facebook
if (!requestData.success || !requestData.data?.platforms?.facebook) {
  console.error('❌ El request de Facebook no fue exitoso o no contiene datos');
  throw new Error('Invalid Facebook response format');
}

const facebookData = requestData.data.platforms.facebook;
const metrics = facebookData.metrics;
const extractedDate = new Date(requestData.data.timestamp);

console.log(`   Plataforma: Facebook`);
console.log(`   Período: ${facebookData.period}`);
